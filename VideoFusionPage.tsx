...
const VideoFusionPage: React.FC<VideoFusionPageProps> = ({ apiKey, onGenerateVideo }) => {
    ...
    const combineImages = (image1: HTMLImageElement, image2: HTMLImageElement): Promise<File> => {
        return new Promise((resolve, reject) => {
            ...
            // Make the canvas wide enough for two images, using the height of the taller image
            const height = Math.max(image1.height, image2.height);
            const width = image1.width + image2.width;
            canvas.width = width;
            canvas.height = height;

            // Draw images, centering the shorter one vertically if necessary
    ...
    const handleGenerateAndNavigate = async () => {
        ...
        showMessage('Menggabungkan gambar dan membuat prompt...', 'info');
        ...
        try {
            ...
            let instructionText = `This is a cinematic video generation task. The provided image is a film strip containing two keyframes, side-by-side. Your task is to create a seamless video that transitions from the left frame to the right frame over approximately ${transition.duration || '4'} seconds. `;
            if (cameraDeltaPrompt) {
                instructionText += cameraDeltaPrompt;
            }
            if (transition.additionalPrompt) {
                instructionText += ` During the transition, incorporate the following creative direction: "${transition.additionalPrompt}".`;
            }
            instructionText += ` The final video should feel like a single, continuous shot.`
            ...
            <div className="bg-black/30 backdrop-blur-sm border border-gray-700 rounded-xl p-6 mt-auto">
                ...
                <div className="flex justify-end items-center mt-6 border-t border-gray-600 pt-4">
                     <button
                        ...
                        disabled={isProcessing || !scene1 || !scene2}
