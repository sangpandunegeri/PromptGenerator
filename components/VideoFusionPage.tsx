...
import { generateVideoFusionPrompt } from '../services/geminiService';
...
const VideoFusionPage: React.FC<VideoFusionPageProps> = ({ apiKey, onGenerateVideo }) => {
    ...
    const [transition, setTransition] = useState<Transition>({ duration: '4', additionalPrompt: '' });
    ...
    const handleGeneratePrompt = async () => {
        ...
        setGeneratedPrompt('');
        showMessage('Menganalisis gambar dan membuat prompt...', 'info');

        try {
            const sceneData = [{ file: scene1.file, camera: scene1.camera }, { file: scene2.file, camera: scene2.camera }];
            const prompt = await generateVideoFusionPrompt(sceneData, transition, apiKey);
            setGeneratedPrompt(prompt);
            showMessage('Prompt transisi sinematik berhasil dibuat!', 'success');
        } catch (error) {
            console.error(error);
    ...
    const handleReset = () => {
        ...
        setScene1(null);
        setScene2(null);
        setTransition({ duration: '4', additionalPrompt: '' });
        setGeneratedPrompt('');
        setMessage('');
    };
    ...
    const handleCameraPreset = (
        ...
        const panStep = 5; // 5% pan step
        const panLimit = 50; // 50% pan limit
        switch (preset) {
    ...
    return (
        <div className="p-6 bg-gray-900 rounded-xl shadow-lg flex flex-col h-full">
            ...
            <div className="text-center mb-8">
                 ...
                 <p className="text-lg text-gray-400 mt-2">Buat prompt untuk transisi mulus antara dua gambar.</p>
            </div>
            ...
            <div className="bg-black/30 backdrop-blur-sm border border-gray-700 rounded-xl p-6 mt-auto">
                ...
                <div className="flex flex-col lg:flex-row items-start gap-6">
                    <div className="flex-grow w-full">
                         <TextAreaField
                            ...
                            value={transition.additionalPrompt}
                            onChange={(e) => handleTransitionChange('additionalPrompt', e.target.value)}
                            rows={3}
                            placeholder="Contoh: tambahkan efek kilat saat transisi, buat suasana menjadi lebih gelap..."
                            tooltip="Berikan instruksi spesifik untuk mengubah konten atau gaya selama transisi."
                        />
                    </div>
                ...
                <div className="flex justify-end items-center mt-6 border-t border-gray-600 pt-4">
                     <button
                        onClick={handleGeneratePrompt}
                        disabled={isGeneratingPrompt || !scene1 || !scene2}
                    ...
                    >
                        ...
                        {isGeneratingPrompt ? 'Membuat...' : 'Hasilkan Prompt'}
                    </button>
                </div>
            </div>

            {generatedPrompt && (
                <div className="bg-black/30 backdrop-blur-sm border border-gray-700 rounded-xl p-6 mt-8">
                    <h3 className="text-2xl font-bold text-white mb-4">Prompt Dihasilkan</h3>
                    <div className="relative bg-gray-900 p-4 rounded-md border border-gray-600 mb-4">
                        ...
                        <button onClick={() => { navigator.clipboard.writeText(generatedPrompt); showMessage('Prompt disalin!', 'success'); }} className="absolute top-2 right-2 p-2 rounded-full bg-gray-600 hover:bg-gray-500"><Copy className="w-4 h-4" /></button>
                    </div>
                    <div className="flex justify-end items-center mt-4">
                        <button
                            onClick={() => onGenerateVideo(generatedPrompt, scene1?.file || null)}
